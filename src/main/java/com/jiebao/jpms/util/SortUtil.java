...
import com.jiebao.jpms.model.QueryRequest;
...
public class SortUtil {
    ...
    public static void handlePageSort(QueryRequest request, Page page, String defaultSort, String defaultOrder, boolean camelToUnderscore) {
        ...
        if (camelToUnderscore) {
            sortField = JiebaoUtil.camelToUnderscore(sortField);
            defaultSort = JiebaoUtil.camelToUnderscore(defaultSort);
        }
        if (StringUtils.isNotBlank(request.getSortField())
            ...
            if (StringUtils.equals(request.getSortOrder(), JiebaoConstant.ORDER_DESC))
                page.setDesc(sortField);
            else
                page.setAsc(sortField);
        } else {
            if (StringUtils.isNotBlank(defaultSort)) {
                if (StringUtils.equals(defaultOrder, JiebaoConstant.ORDER_DESC))
